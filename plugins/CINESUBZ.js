...
async function getCineSubzLinks(originalUrl) {
  ...
  for (let serverNum of serversToTry) {
    ...
    try {
      ...
      const baseHeaders = { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)', 'Accept': 'text/html,application/xhtml+xml', 'Connection': 'keep-alive' };

      const initialRes = await axios.get(movieUrl, { httpsAgent: agent, headers: baseHeaders });
      let cookieHeader = '';
      if (initialRes.headers['set-cookie']) cookieHeader = initialRes.headers['set-cookie'].map(c => c.split(';')[0]).join('; ');
      ...
      if (payloads.length === 0) {
        ...
        const apiResponse = await axios.get(apiUrl, { httpsAgent: agent, headers: { ...baseHeaders, 'Accept': 'application/json', 'Referer': movieUrl, 'Cookie': cookieHeader } });
        if (apiResponse.headers['set-cookie']) cookieHeader = apiResponse.headers['set-cookie'].map(c => c.split(';')[0]).join('; ');
        ...
        const pageResponse = await axios.get(realPageUrl, { httpsAgent: agent, headers: { ...baseHeaders, 'Referer': movieUrl, 'Cookie': cookieHeader } });
      ...
      for (let hexPayload of payloads) {
        try {
          ...
          const dlResponse = await axios.post(realPageUrl, payloadBytes, { httpsAgent: agent, headers: { 'Content-Type': 'application/octet-stream', 'Referer': realPageUrl, 'Cookie': cookieHeader }, responseType: 'arraybuffer' });
            ...
            for (let key of uniqueStrings) {
              try {
                ...
                if (decodedStr) {
                  ...
                  if (finalUrl.startsWith('http')) { results.push(finalUrl); break; }
                }
              } catch (e) {}
            }
          }
    ...
    if (pending.step === 1) {
      ...
      try {
        ...
        if (!movieInfo || !movieInfo.downloadLinks || movieInfo.downloadLinks.length === 0) {
          clearUserSession(k); return await sendErrorMsg(sock, from, mek, "No download links available for this movie.");
        }
        ...
        if (downloadLinks.length === 0) { clearUserSession(k); return await sendErrorMsg(sock, from, mek, "No download links found below 2GB."); }
        ...
        if (movieInfo.poster) { await sock.sendMessage(from, { image: { url: movieInfo.poster }, caption: qualityMsg, contextInfo: channelContextInfo() }, { quoted: mek }); } 
        else { await sock.sendMessage(from, { text: qualityMsg, contextInfo: channelContextInfo() }, { quoted: mek }); }
        
        pending.step = 2; pending.movie = { metadata: movieInfo, downloadLinks }; pending.timestamp = Date.now(); pending.isProcessing = false;
        await sock.sendMessage(from, { react: { text: "✅", key: m.key } });
      } catch (error) { clearUserSession(k); await sendErrorMsg(sock, from, mek, "Failed to fetch download links for this movie."); }
    }
      ...
      try {
        ...
        if (!finalResult.success) return await sendErrorMsg(sock, from, mek, `Error Extracting Link: ${finalResult.error}`);
        ...
        let directDownloadUrl = skylineLinks.length > 0 ? skylineLinks[0] : (pixeldrainLinks.length > 0 ? pixeldrainLinks[0] : null);
        ...
        await sock.sendMessage(from, { react: { text: "✅", key: m.key } });
      } catch (error) { await sendErrorMsg(sock, from, mek, `Failed to download movie: ${error.message}`); }
    }
  }
};
