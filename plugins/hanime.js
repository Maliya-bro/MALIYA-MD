...
cmd(
  ...
  async (sock, mek, m, { from, args, reply }) => {
    try {
      ...
      let listText = `*╭───[ 🔞 𝗠𝗔𝗟𝗜𝗬𝗔-𝗠𝗗 𝗛𝗔𝗡𝗜𝗠𝗘 𝗦𝗘𝗔𝗥𝗖𝗛 ]───╮*\n│\n├─ 🎯 *🎯 Query:* _${hQuery}_\n├─ 📊 *📊 Results:* _${hResults.length} Items_\n│\n├─ *👇 Reply with a Number:* 👇\n│\n`;
...
replyHandlers.push({
  filter: (body, { from }) => {
    return !isNaN(parseInt(body.trim()));
  },
  function: async (sock, mek, m, { from, body, reply }) => {
    const contextInfo = mek.message?.extendedTextMessage?.contextInfo;
    const quotedId = contextInfo?.stanzaId;

    if (!quotedId || !activeHanimeSessions.has(quotedId)) return;

    const sessionData = activeHanimeSessions.get(quotedId);

    // Verify chat environment match
    if (sessionData.sender !== from) return;
    ...
    if (sessionData.step === "SELECT_SEARCH") {
      ...
      if (isNaN(choiceNum) || choiceNum < 0 || choiceNum >= hResults.length) {
        return await reply(
      ...
      try {
        ...
        let downloadsListText = `*╭───[ 🔞 𝗠𝗔𝗟𝗜𝗬𝗔-𝗠𝗗 𝗩𝗜𝗗𝗘𝗢 𝗜𝗡𝗙𝗢 ]───╮*\n│\n`;
        downloadsListText += `├─ 🎬 *𝗧𝗶𝘁𝗹𝗲:* ${videoInfo.title}\n`;
        downloadsListText += `├─ ⭐ *𝗥𝗮𝘁𝗶𝗻𝗴:* ★ ${videoInfo.rating || "N/A"}\n`;
        downloadsListText += `├─ 📅 *𝗬𝗲𝗮𝗿:* ${videoInfo.year || "N/A"}\n`;
        downloadsListText += `├─ 🌍 *𝗖𝗼𝘂𝗻𝘁𝗿𝘆:* ${videoInfo.country || "N/A"}\n`;
        downloadsListText += `├─ 🎭 *𝗚𝗲𝗻𝗿𝗲𝘀:* ${videoInfo.genres ? videoInfo.genres.join(", ") : "N/A"}\n`;
        downloadsListText += `├─ 📝 *𝗦𝘁𝗼𝗿𝘆:* ${videoInfo.story ? (videoInfo.story.length > 200 ? videoInfo.story.substring(0, 200) + "..." : videoInfo.story) : "N/A"}\n│\n`;
        ...
        activeHanimeSessions.delete(quotedId);
    ...
    // STEP 2: Process Download Selection -> Send Video Document
      ...
      try {
        ...
        const captionText = `*╭───[ 🔞 𝗠𝗔𝗟𝗜𝗬𝗔-𝗠𝗗 𝗛𝗔𝗡𝗜𝗠𝗘 ]───╮*\n│\n├─ 🎭 *𝗧𝗶𝘁𝗹𝗲:* ${videoInfo.title}\n├─ 📊 *𝗤𝘂𝗮𝗹𝗶𝘁𝘆:* ${selectedDownload.title || selectedDownload.name || "720p HD Direct MP4"}\n│\n╰──────────────────────────╯${DEFAULT_FOOTER}`;
