...
// MediaFire Direct Link Extractor for Subtitles
  ...
  function: async (sock, mek, m, { body, sender, from }) => {
    ...
    if (pending.step === 1) {
      ...
      try {
        ...
        if (isSeries) {
          // සීරීස් එකක් නම් Episode එක අහනවා
          pending.step = 2;
...
async function executeMovieSubDownload(sock, mek, from, videoUrl, subUrl, titleName, fullLinksObj, posterImg) {
  ...
  try {
    ...
    // Google Drive වයිරස් පේජ් එක හරි වෙනත් HTML පේජ් එකක් හරි ආවොත් එතනින්ම නවත්තනවා (Fallback එකට යවනවා)
    ...
    const docPayload = {
      document: fs.readFileSync(tempVideo),
    ...
    if (subUrl && subUrl !== '#') {
        ...
        // Subtitle එක යැවීම
        await sock.sendMessage(from, {
            document: fs.readFileSync(tempSub),
    ...
    // 🔥 Fallback: Error එකක් ආවොත් (2GB පන්නලා හරි GDrive අවුලක් හරි) කෙලින්ම ලින්ක්ස් ටික යවනවා.
    let fallbackMsg = `⊱━━━━━ • ✿ • ━━━━━⊰\n`;
    ...
    if (videoUrl && videoUrl !== '#') {
        ...
        if (fullLinksObj.gdrive && fullLinksObj.gdrive.length > 0) {
            fallbackMsg += `🔗 *G-Drive:*\n${fullLinksObj.gdrive[0]}\n\n`;
        }
        if (fullLinksObj.telegram && fullLinksObj.telegram.length > 0) {
